...
public class VeiculosJpaController implements Serializable {
	...
	public List<VEICULOS> findVeiculosByMorador(String moradorID) {
        ...
        try {
            ...
            Query query = em.createNativeQuery("SELECT * FROM veiculos WHERE IDMORADOR = ? AND VISITANTE IS NULL", VEICULOS.class);
            query.setParameter(1, moradorID);
            List<VEICULOS> resultList = query.getResultList();
            cnt.close();
            return resultList;
	...
	public List<VEICULOS> findVeiculosByVisitante(Long id) {
        ...
        try {
            em.getTransaction().begin();
            Connection cnt = em.unwrap(Connection.class);
            Query query = em.createNativeQuery("SELECT * FROM veiculos WHERE VISITANTE = ?", VEICULOS.class);
